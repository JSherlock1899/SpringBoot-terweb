...
$(document).on("click",".update",function(e,url){
	...
	$("#name").attr("value",$(this).closest("tr").find(".name").text());
	$("#Tsex").attr("value",$(this).closest("tr").find(".tsex").text());
    $('#Tdegree option:selected') .val($(this).closest("tr").find(".tdegree").val());
	...
	$("#dname").attr("value",$(this).closest("tr").find(".dname").val());
});
...
$(document).on("click",".save",function(e,url){
	...
	var Cname = $('#terCollege option:selected').val();
	var Dname = $('#terSdept option:selected').val();
	$.ajax({
		url:"/teacher/update",
		type:"post",
		datatype:"json",
