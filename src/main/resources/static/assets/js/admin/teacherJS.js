function terMove(){								//实现各院系的两级联动
    //院管理员所属学院
    var college = $('#codCname').val();
    var collegeName;
    if(college == "null" || college == null || college == ""){
        collegeName = $('#terCollege option:selected').val();
    }else{
        collegeName = college;
    }
    var sdept = document.getElementById("terSdept");
    var add;
    //用来存储各学院的专业信息
    ...
    sdept.length = 0;
    var codDname = $("#codDname").val();
    for(var i =0;i<add.length;i++){     //将对应的专业添加到二级select中
        var newSdept = new Option();
        if (add[i].split()[0] == codDname){
            newSdept.selected = true;
        }
