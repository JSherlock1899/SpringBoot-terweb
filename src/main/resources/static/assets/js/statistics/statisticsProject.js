...
$(function() {
    ...
    if(cname == null || cname == "null"){
        for (var i = 0;i<Pmoneyjson.length;i++) {
            keyArr.push(Pmoneyjson[i].name); // 添加学院名
            valueArr.push(Pmoneyjson[i].money) // 添加项目经费
            jsonArr.push({
                name: Pmoneyjson[i].name,
                value: Pmoneyjson[i].money
            })
        }
    }else{
        for (var i = 0;i<Pmoneyjson.length;i++) {
            keyArr.push(Pmoneyjson[i].name); // 添加学院名
            valueArr.push(Pmoneyjson[i].money) // 添加项目数
            jsonArr.push({
                name: Pmoneyjson[i].name,
                value: Pmoneyjson[i].money
            })
